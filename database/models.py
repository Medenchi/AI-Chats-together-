...
class Database:
    ...
    async def add_character(self, name, age, personality, model,
        ...
        async with aiosqlite.connect(self.path) as db:
            c = await db.execute(
                "INSERT INTO characters (name,age,personality,model,bot_token,bot_username,bot_id,topic_id,parent_character_id) VALUES (?,?,?,?,?,?,?,?,?)",
    ...
    async def get_character(self, cid):
        async with aiosqlite.connect(self.path) as db:
            ...
            r = await (await db.execute("SELECT * FROM characters WHERE id=?", (cid,))).fetchone()
            return dict(r) if r else None

    async def get_all_characters(self, active=True):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            q = "SELECT * FROM characters" + (" WHERE is_active=1" if active else "")
    ...
    async def add_memory(self, cid, content, mt="conversation", imp=0.5):
        async with aiosqlite.connect(self.path) as db:
            await db.execute("INSERT INTO memories (character_id,content,memory_type,importance) VALUES (?,?,?,?)",
                             (cid, content, mt, imp))
            await db.commit()
    ...
    async def add_conversation(self, fc, tc, msg, tid=None, is_bot=False):
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "INSERT INTO conversations (from_character_id,to_character_id,topic_id,message,is_bot_message) VALUES (?,?,?,?,?)",
    ...
    async def get_conversations(self, cid=None, tid=None, limit=20):
        async with aiosqlite.connect(self.path) as db:
            ...
            if cid:
                q += " AND (from_character_id=? OR to_character_id=?)"; p += [cid, cid]
            if tid:
                q += " AND topic_id=?"; p.append(tid)
            q += " ORDER BY created_at DESC LIMIT ?"; p.append(limit)
            return [dict(r) for r in await (await db.execute(q, p)).fetchall()]
    ...
    async def update_relationship(self, c1, c2, rt=None, cl=None):
        async with aiosqlite.connect(self.path) as db:
            r = await (await db.execute(
                "SELECT id,closeness FROM relationships WHERE character1_id=? AND character2_id=?", (c1, c2))).fetchone()
            if r:
                u, v = [], []
                if rt is not None:
                    u.append("relationship_type=?"); v.append(rt)
                if cl is not None:
                    u.append("closeness=?"); v.append(cl)
                if u:
                    v.append(r[0])
                    await db.execute(f"UPDATE relationships SET {','.join(u)} WHERE id=?", v)
            else:
                await db.execute(
                    "INSERT INTO relationships (character1_id,character2_id,relationship_type,closeness) VALUES (?,?,?,?)",
    ...
    async def get_relationship(self, c1, c2):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            r = await (await db.execute(
                "SELECT * FROM relationships WHERE character1_id=? AND character2_id=?", (c1, c2))).fetchone()
            return dict(r) if r else None

    async def save_state(self, cid, key, val):
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "INSERT INTO bot_states (character_id,state_key,state_value) VALUES (?,?,?) "
                "ON CONFLICT(character_id,state_key) DO UPDATE SET state_value=?, updated_at=CURRENT_TIMESTAMP",
                (cid, key, val, val))
            await db.commit()
    ...
    async def get_state(self, cid, key):
        async with aiosqlite.connect(self.path) as db:
            r = await (await db.execute(
                "SELECT state_value FROM bot_states WHERE character_id=? AND state_key=?", (cid, key))).fetchone()
            return r[0] if r else None

    async def add_topic(self, tid, tname, cbid=None):
        async with aiosqlite.connect(self.path) as db:
            await db.execute("INSERT OR IGNORE INTO topics (topic_id,topic_name,created_by_character_id) VALUES (?,?,?)",
                             (tid, tname, cbid))
            await db.commit()

    async def update_character_topic(self, cid, tid):
        async with aiosqlite.connect(self.path) as db:
            await db.execute("UPDATE characters SET topic_id=? WHERE id=?", (tid, cid))
            await db.commit()

    async def get_topic(self, tid):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            r = await (await db.execute("SELECT * FROM topics WHERE topic_id=?", (tid,))).fetchone()
            return dict(r) if r else None

    async def update_bot_token(self, cid, token, username, bid=None):
        async with aiosqlite.connect(self.path) as db:
            await db.execute("UPDATE characters SET bot_token=?, bot_username=?, bot_id=? WHERE id=?",
    ...
    async def get_character_by_username(self, username):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            r = await (await db.execute("SELECT * FROM characters WHERE bot_username=?", (username,))).fetchone()
            return dict(r) if r else None
