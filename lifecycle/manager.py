...
class LifecycleManager:
    ...
    async def _clone_loop(self):
        while self.is_running:
            try:
                for ch in await self.db.get_all_characters(active_only=True):
    ...
    async def _should_clone(self, ch: Dict[str, Any]) -> bool:
        ...
        cnt = sum(1 for c in clones if c.get("parent_character_id") == ch["id"])
        if cnt >= self.master_bot.config.get("MAX_CLONES", 3):
            return False
        if len(clones) >= self.master_bot.config.get("MAX_BOTS", 10):
            return False
        mood_w = {"happy": 0.3, "energetic": 0.25, "neutral": 0.1, "grumpy": 0.05, "sleepy": 0.0}
        return random.random() < mood_w.get(ch.get("mood", "neutral"), 0.1)

    async def _clone(self, parent: Dict[str, Any]):
        pp = eval(parent.get("personality", "{}"))
        cp = await self.pg.generate_personality(model=parent["model"], age=pp.get("age", 16))
        cp["traits"].append(f"похож на {pp['name']}")
        try:
            cid = await self.db.add_character(
                name=cp["name"], age=cp["age"], personality=str(cp),
                model=parent["model"], parent_character_id=parent["id"],
            )
            try:
                await self.master_bot.master_bot.send_message(
                    chat_id=self.master_bot.group_id,
                    text=f"🎉 {pp['name']} привёл нового друга — {cp['name']} ({cp['age']} лет)!",
                )
            except Exception:
                pass
    ...
    async def _check_rels(self, ch: Dict[str, Any]):
        for other in await self.db.get_all_characters(active_only=True):
            if other["id"] == ch["id"]:
                continue
            rel = await self.db.get_relationship(ch["id"], other["id"])
            if rel and rel["closeness"] > 0.8 and rel["relationship_type"] == "friend":
                await self.db.update_relationship(ch["id"], other["id"], rt="close_friend")
                logger.info("💕 %s & %s → close friends!", ch["name"], other["name"])
                p1 = eval(ch.get("personality", "{}"))
                p2 = eval(other.get("personality", "{}"))
                try:
                    await self.master_bot.master_bot.send_message(
                        chat_id=self.master_bot.group_id,
                        text=f"💕 {p1['name']} и {p2['name']} стали очень близкими друзьями!",
                    )
                except Exception:
                    pass
    ...
    async def _interact(self, c1: Dict[str, Any], c2: Dict[str, Any]):
        ...
        try:
            resp = await self.ai_client.generate(
                model=c1["model"],
                messages=[{"role":"user","content":f"{p1['name']} говорит что-то дружеское (1-2 предложения, без эмодзи)."}],
                max_tokens=80, temperature=0.8,
            )
            await bot1.bot.send_message(
                chat_id=self.master_bot.group_id,
                text=f"{p1['name']}: {resp.strip()}",
    ...
    async def _bot_to_bot_loop(self):
        ...
        while self.is_running:
            try:
                active = [b for b in self.master_bot.bots.values() if not b.is_sleeping]
                if len(active) >= 2 and random.random() < 0.1:
                    c1 = random.choice(active)
                    others = [b for b in active if b.character_id != c1.character_id]
                    c2 = random.choice(others)

                    logger.info("🗣 Bot-to-bot: %s → %s", c1.personality["name"], c2.personality["name"])
                    ...
                    if reply:
                        ...
                        for i in range(turns):
                            ...
                            await speaker.reply_to_character(
    ...
    async def create_topic(self, character_id: int, name: str):
        try:
            ...
            await self.db.add_topic(topic.message_thread_id, name, character_id)
            await self.db.update_character_topic(character_id, topic.message_thread_id)
            logger.info("📂 Topic '%s' created (thread_id=%d)", name, topic.message_thread_id)
